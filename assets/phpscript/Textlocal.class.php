...
class Textlocal
{
	 ...
	 * @throws Exception
	 */
	public function sendSmsGroup($grouCID, $message, $sender = null, $sched = null, $test = false, $receiptURL = null, $custom = null, $optouts = false, $simpleReplyService = false)
	{

		if (!is_numeric($grouCID))
			throw new Exception('Invalid $grouCID format. Must be a numeric group ID');
		if (empty($message))
			throw new Exception('Empty message');
		if (empty($sender))
			throw new Exception('Empty sender name');
	 ...
	 * @throws Exception
	 */
	public function sendMmsGroup($grouCID, $fileSource, $message, $sched = null, $test = false, $optouts = false)
	{
	 ...
	 * @throws Exception
	 */
	public function getContacts($grouCID, $limit, $startPos = 0)
	{
	 ...
	 * @param        $numbers
	 * @param string $grouCID
	 * @return array|mixed
	 */
	public function createContacts($numbers, $grouCID = '5')
	{
		$params = array("group_id" => $grouCID);
	 ...
	 * @return array|mixed
	 */
	function createContactsBulk($contacts, $grouCID = '5')
	{
		// JSON & URL-encode array
		$contacts = urlencode(json_encode($contacts));

		$params = array
		("group_id" => $grouCID, "contacts" => $contacts);
	 ...
	 * @return array|bool|mixed
	 */
	public function deleteContact($number, $grouCID = 5)
	{
		if (!isset($number)) return false;
		$options = array('number' => $number, 'group_id' => $grouCID);
	 ...
	 * @return array|mixed
	 */
	public function deleteGroup($grouCID)
	{
		$options = array('group_id' => $grouCID);
